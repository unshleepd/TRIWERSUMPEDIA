...
class GameLogic:
    ...
    def get_dialogues(self):
        return {
            "ji_ae_start": {"text": "Witaj, wędrowcze.", "options": [{"label": "Lord Bonglord mnie przysyła.", "next": "ji_ae_quest_talk", "condition": lambda: self.player.quest_journal.get("bonglord_quest", {}).status == "active"}, {"label": "[Odejdź]", "next": "end"}]},
            "ji_ae_quest_talk": {"text": "Ach, więc to o to chodzi. Siewca Paradygmatu na północy... Jest potężny. Uważaj.", "options": [{"label": "[Dziękuję za ostrzeżenie]", "next": "end", "action": "update_quest", "target": "Mędrzec Ji-Ae"}]},
            "bonglord_start": {"text": "Czuję harmonię w twoim oddechu. Ale coś zakłóca przepływy...", "options": [{"label": "Co masz na myśli?", "next": "bonglord_quest_offer", "condition": lambda: self.player.quest_journal.get("bonglord_quest") is None}, {"label": "Wróciłem, mistrzu.", "next": "bonglord_reward", "condition": lambda: self.player.quest_journal.get("bonglord_quest", {}).status == "completed"}, {"label": "[Odejdź]", "next": "end"}]},
            "bonglord_quest_offer": {"text": "Siewca Paradygmatu mąci na północy. Porozmawiaj z Ji-Ae, a potem go pokonaj. Zostaniesz nagrodzony.", "options": [{"label": "Zgoda.", "next": "end", "action": "start_quest", "quest_key": "bonglord_quest"}, {"label": "Nie teraz.", "next": "end"}]},
            "bonglord_reward": {"text": "Wykonałeś zadanie. Równowaga wraca. Przyjmij ten dar.", "options": [{"label": "[Weź nagrodę]", "next": "end", "action": "grant_reward", "quest_key": "bonglord_quest"}]}
        }
...
def main():
    ...
    except tk.TclError: print("Could not open display. This is expected in a headless environment.")
